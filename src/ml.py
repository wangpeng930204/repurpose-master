...
def check_ml(data, n_run, knn, n_fold, n_proportion, n_subset, model_type, prediction_type, features,
             recalculate_similarity, disjoint_cv, split_both=False, output_file=None, model_fun=None, verbose=False,
             n_seed=None):
    ...
    if prediction_type == "disease":
        ...
        disease_to_drugs, pairs, classes = utilities.get_drug_disease_mapping(drugs, drug_to_values_interaction,
                                                                              drug_interaction_to_index)
    else:
    ...
    if output_file is not None:
        ...
        output_f.write(
            "n_fold\tn_proportion\tn_subset\tmodel type\tprediction type\tfeatures\trecalculate\tdisjoint\tpairwise\tvariable\tauc.mean\tauc.sd\tauprc.mean\tauprc.sd\n")
    else:
        output_f = None
    values = []
    values2 = []
    for i in range(n_run):
        ...
        pairs_, classes_, cv = utilities.balance_data_and_get_cv(pairs, classes, n_fold, n_proportion, n_subset,
                                                                 disjoint=disjoint_cv, split_both=split_both,
                                                                 n_seed=n_seed)
        val, val2 = check_ml_helper(drugs, disease_to_drugs, drug_to_index, list_M_similarity, pairs_, classes_, cv,
                                    knn, n_fold, n_proportion, n_subset, model_type, prediction_type, features,
                                    recalculate_similarity, disjoint_cv, split_both, output_f, model_fun, verbose,
                                    n_seed)
        values.append(val)
        values2.append(val2)
    print("AUC over runs: %.1f (+/-%.1f):" % (numpy.mean(values), numpy.std(values)),
          [round(x, ndigits=1) for x in values])
    if output_f is not None:
        output_f.write("%d\t%d\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%f\t%f\t%f\t%f\n" % (
            n_fold, n_proportion, n_subset, model_type, prediction_type, "|".join(features), recalculate_similarity,
            disjoint_cv, split_both, "avg", numpy.mean(values), numpy.std(values), numpy.mean(values2),
            numpy.std(values2)))
        output_f.close()
    return "AUC: %.1f" % numpy.mean(values), "AUPRC: %.1f" % numpy.mean(values2)


def check_ml_helper(drugs, disease_to_drugs, drug_to_index, list_M_similarity, pairs, classes, cv, knn, n_fold,
                    n_proportion, n_subset, model_type, prediction_type, features, recalculate_similarity, disjoint_cv,
                    split_both, output_f, model_fun, verbose, n_seed):
    ...
    if recalculate_similarity:
        drug_to_disease_to_scores = utilities.get_similarity_based_scores(drugs, disease_to_drugs, drug_to_index,
                                                                          list_M_similarity=list_M_similarity,
                                                                          knn=knn, pairs_train=pairs_train,
                                                                          pairs_test=None,
                                                                          approach="train_vs_train",
                                                                          file_name=file_name)
    else:
        ...
        drug_to_disease_to_scores = utilities.get_similarity_based_scores(drugs, disease_to_drugs, drug_to_index,
                                                                          ...
                                                                          approach="train_test_vs_train_test",
                                                                          file_name=file_name)  # similar to all_vs_all above, but removes the test pair
    X, y = get_scores_and_labels(pairs_train, classes_train, drug_to_disease_to_scores)
    if recalculate_similarity:
        drug_to_disease_to_scores = utilities.get_similarity_based_scores(drugs, disease_to_drugs, drug_to_index,
                                                                          list_M_similarity=list_M_similarity,
                                                                          knn=knn, pairs_train=pairs_test,
                                                                          pairs_test=None,
                                                                          approach="train_vs_train",
                                                                          file_name=file_name)
    ...
    # if verbose:
    #     print("Fold:", i + 1, "# train:", len(pairs_train), "# test:", len(pairs_test), "AUC: %.1f" % roc_auc,
    #           "AUPRC: %.1f" % prc_auc)
    ...
    if output_f is not None:
        output_f.write("%d\t%d\t%d\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%f\t%f\t%f\t%f\n" % (
            n_fold, n_proportion, n_subset, model_type, prediction_type, "|".join(features), recalculate_similarity,
            disjoint_cv, split_both, "cv", numpy.mean(all_auc), numpy.std(all_auc), numpy.mean(all_auprc),
            numpy.std(all_auprc)))
