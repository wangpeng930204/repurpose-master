...
def balance_data_and_get_cv(pairs, classes, n_fold, n_proportion, n_subset=-1, disjoint=False, split_both=False,
                            n_seed=None):
    """
    ...
    if disjoint:
        ...
        def get_groups(idx_true_list, idx_false_list, n_subset, n_proportion=1, shuffle=False):
            ...
            for i in range(n):
                if n_subset == -1:  # use all data
                    ...
                    if shuffle:
                        indices_test = random.sample(idx_true_list[i], n_subset) + random.sample(idx_false_list[i],
                                                                                                 n_proportion * n_subset)
                    else:
                ...
                for j in range(n):
                    ...
                    if n_subset == -1:  # use all data
                        ...
                        if shuffle:
                            indices_train += random.sample(idx_true_list[j], n_subset) + random.sample(
                                idx_false_list[j], n_proportion * n_subset)
                        else:
        ...
        if split_both:
            id_to_fold = {}
            drugs = reduce(lambda x, y: set(x + y), list(zip(*pairs)))
            ...
            if any(numpy.array(list(map(len, idx_true_list))) == 0) or any(
                    numpy.array(list(map(len, idx_false_list))) == 0):
                print(list(map(len, idx_true_list)), list(map(len, idx_false_list)))
        ...
        # cv = StratifiedKFold(classes, shuffle=True, random_state=n_seed)
        cv = train_test_split(pairs, classes, random_state=True, shuffle=True, test_size=0.3)
...
def get_classification_model(model_type, model_fun=None, n_seed=None):
    ...
    if model_type == "svm":
        ...
        clf = linear_model.LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0,
                                              random_state=n_seed)  # , fit_intercept=True, intercept_scaling=1, class_weight=None, solver='liblinear', max_iter=100, multi_class='ovr', verbose=0, warm_start=False, n_jobs=1)
    elif model_type == "knn":
        ...
        clf = tree.DecisionTreeClassifier(criterion='gini',
                                          random_state=n_seed)  # splitter='best', max_depth=None, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features=None, max_leaf_nodes=None, class_weight=None, presort=False)
    elif model_type == "rf":
        clf = ensemble.RandomForestClassifier(n_estimators=10, criterion='gini',
                                              random_state=n_seed)  # , max_depth=None, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_features='auto', max_leaf_nodes=None, bootstrap=True, oob_score=False, n_jobs=1, verbose=0, warm_start=False, class_weight=None)
    elif model_type == "gbc":
        clf = ensemble.GradientBoostingClassifier(n_estimators=100, loss='deviance', learning_rate=0.1, subsample=1.0,
                                                  random_state=n_seed)  # , min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0.0, max_depth=3, init=None, max_features=None, verbose=0, max_leaf_nodes=None, warm_start=False, presort='auto')
...
def rescale_data(X):
    X_new = preprocessing.scale(X, axis=0, with_mean=True, with_std=True, copy=True)
    return X_new


def get_data(drug_disease_file, drug_side_effect_file, drug_structure_file, drug_target_file,
             drug_interaction_file=None):
    """
...
def get_drug_disease_mapping(drugs, drug_to_values, disease_to_index):
    ...
    drugs_all = reduce(lambda x, y: x | y, list(disease_to_drugs.values()))
    for disease, drugs_gold in disease_to_drugs.items():
        for drug in drugs_all:
            ...
            if drug in drugs_gold:
                flag = 1
            classes.append(flag)
    print(len(drugs), "drugs,", len(disease_to_drugs), "diseases,", len(pairs), "pairs,", sum(classes),
          "known associations")
...
def get_similarity(drugs, drug_to_values, drugs_test=None):
    ...
    if drugs_test is not None:
        ...
        M = numpy.matrix(
            [list(map(float, drug_to_values[drug][0])) if drug not in drugs_test else [0.0] * n_feature for drug in
             drugs])
    else:
        M = numpy.matrix([list(map(float, drug_to_values[drug][0])) for drug in drugs])
...
def get_similarity_based_scores(drugs, disease_to_drugs, drug_to_index, list_M_similarity, knn=20, pairs_train=None,
                                pairs_test=None, approach="all_vs_all", file_name=None):
    if file_name is not None and os.path.exists(file_name):
        drug_to_disease_to_scores = pickle.load(open(file_name))
    else:
        ...
        if approach == "all_vs_all":
            ...
            # drugs = set(zip(*pairs_train)[0])
            drugs = set([x[0] for x in pairs_train])
            pairs_train = set((k, v) for k, v in pairs_train)
            for drug1 in drugs:
                idx1 = drug_to_index[drug1]
                drug_to_disease_to_scores[drug1] = {}
        ...
        if file_name is not None:
            pickle.dump(drug_to_disease_to_scores, open(file_name, 'w'))
