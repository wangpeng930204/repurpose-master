...
class TsvReader(FormattedFileProcessor):
    ...
    def read(self, fields_to_include, keys_to_include=None, merge_inner_values=False):
        ...
        columns = dict(list(zip(cols, list(range(len(cols))))))
                ...
                if keys_to_include is None or id_ in keys_to_include:
                    ...
                    if merge_inner_values:
                        for f in fields_to_include:
                            new_vals.extend([x.strip().strip(self.quotation) for x in vals[columns[f]].split(self.inner_delim)])
                    else:
        ...
        column_to_index = dict(list(zip(fields_to_include, list(range(len(fields_to_include))))))
        #print "End:", columns
            ...
            if fields_to_include is not None:
                ...
                for c in cols:
                    if c in fields_to_include:
                        cols2.append(c)
                columns = dict(list(zip(cols2, list(range(len(cols2))))))
            #print "End:", columns
